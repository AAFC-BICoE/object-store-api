...
echo "application = $APPLICATION, database = $POSTGRES_DB, schema = $(echo '$spring.liquibase.defaultSchema' | awk  -f envSubstitution.awk)"
...
if [ "$STATUS" = "3" ]; then
   ...
   awk -f envSubstitution.awk createSchema.sql
   awk -f envSubstitution.awk createSchema.sql | psql -h $POSTGRES_HOST -d $POSTGRES_DB -U $(echo '$spring.liquibase.user' | awk  -f envSubstitution.awk)
   
   export PGPASSWORD="$POSTGRES_PASSWORD"
   awk -f envSubstitution.awk setSearchPath.sql
   awk -f envSubstitution.awk setSearchPath.sql | psql  -h $POSTGRES_HOST -d $POSTGRES_DB -U $POSTGRES_USER
