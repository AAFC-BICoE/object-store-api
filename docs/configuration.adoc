...
| `FS`, `S3` or `MINIO` (deprecated)

|`dina.fileStorage.root`
| Only required for `FS`. Represents the root folder in the context of the container/pod.

|`s3.*`
| Only required for `S3` and `MINIO`. Represents the s3 specific configuration.
|===
