...
import ca.gc.aafc.objectstore.api.mapper.ObjectStoreControlledVocabularyMapper;
...
public class ObjectStoreControlledVocabularyRepository extends DinaRepositoryV2<ObjectStoreControlledVocabularyDto, ObjectStoreControlledVocabulary> {

  // Bean does not exist with keycloak disabled.
  private final DinaAuthenticatedUser authenticatedUser;

  private final ObjectStoreControlledVocabularyService collectionControlledVocabularyService;

  public ObjectStoreControlledVocabularyRepository(
    @NonNull ObjectStoreControlledVocabularyService dinaService,
    ...
    super(
      ...
      ObjectStoreControlledVocabularyMapper.INSTANCE,
      ObjectStoreControlledVocabularyDto.class,
  ...
  @Override
  protected Link generateLinkToResource(ObjectStoreControlledVocabularyDto dto) {
    try {
      return linkTo(methodOn(ObjectStoreControlledVocabularyRepository.class).onFindOne(dto.getUuid().toString(), null)).withSelfRel();
    } catch (ResourceNotFoundException | ResourceGoneException e) {
      throw new RuntimeException(e);
    }
  }

  @GetMapping(ObjectStoreControlledVocabularyDto.TYPENAME + "/{idOrKey}")
  public ResponseEntity<RepresentationModel<?>> onFindOne(@PathVariable String idOrKey, HttpServletRequest req)
    ...
    if (StringUtils.isBlank(idOrKey)) {
      throw ResourceNotFoundException.create(ObjectStoreControlledVocabularyDto.TYPENAME, "");
    }
    ...
    ObjectStoreControlledVocabulary entity = collectionControlledVocabularyService.findOneByKey(idOrKey);
    if (entity == null) {
      throw ResourceNotFoundException.create(ObjectStoreControlledVocabularyDto.TYPENAME, idOrKey);
    }
    return handleFindOne(entity.getUuid(), req);
  }

  @PostMapping(path = ObjectStoreControlledVocabularyDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH,
  ...
  @GetMapping(ObjectStoreControlledVocabularyDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = ObjectStoreControlledVocabularyDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(ObjectStoreControlledVocabularyDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(path = ObjectStoreControlledVocabularyDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PatchMapping(ObjectStoreControlledVocabularyDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @DeleteMapping(path = ObjectStoreControlledVocabularyDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(ObjectStoreControlledVocabularyDto.TYPENAME + "/{id}")
  @Transactional
