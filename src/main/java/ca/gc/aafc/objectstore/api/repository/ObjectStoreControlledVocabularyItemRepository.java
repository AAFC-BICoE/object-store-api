...
public class ObjectStoreControlledVocabularyItemRepository extends DinaRepositoryV2<ObjectStoreControlledVocabularyItemDto, ObjectStoreControlledVocabularyItem> {

  // Bean does not exist with keycloak disabled.
  private final DinaAuthenticatedUser authenticatedUser;

  private final ObjectStoreControlledVocabularyService controlledVocabularyService;
  private final ObjectStoreControlledVocabularyItemService controlledVocabularyItemService;

  public ObjectStoreControlledVocabularyItemRepository(
    @NonNull ObjectStoreControlledVocabularyItemService controlledVocabularyItemService,
    @NonNull ObjectStoreControlledVocabularyService controlledVocabularyService,
    ...
    super(
      ...
      ObjectStoreControlledVocabularyItemMapper.INSTANCE,
      ObjectStoreControlledVocabularyItemDto.class,
      ObjectStoreControlledVocabularyItem.class,
      props, objMapper);
  ...
  @Override
  protected Link generateLinkToResource(ObjectStoreControlledVocabularyItemDto dto) {
    try {
      return linkTo(methodOn(ObjectStoreControlledVocabularyItemRepository.class).onFindOne(dto.getUuid().toString(), null)).withSelfRel();
  ...
  public ResponseEntity<RepresentationModel<?>> onFindOne(@PathVariable String idOrKey, HttpServletRequest req)
    ...
    if (keyParts.length == 2 || keyParts.length == 3) {
      ObjectStoreControlledVocabulary vocab = controlledVocabularyService.findOneByKey(keyParts[0]);
      if (vocab != null) {
        ObjectStoreControlledVocabularyItem item = controlledVocabularyItemService.findOneByKey(keyParts[1], vocab.getUuid(),
  ...
  @PostMapping(path = ObjectStoreControlledVocabularyItemDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH,
  ...
  @GetMapping(ObjectStoreControlledVocabularyItemDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = ObjectStoreControlledVocabularyItemDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(ObjectStoreControlledVocabularyItemDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(ObjectStoreControlledVocabularyItemDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @DeleteMapping(path = ObjectStoreControlledVocabularyItemDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(ObjectStoreControlledVocabularyItemDto.TYPENAME + "/{id}")
  @Transactional
